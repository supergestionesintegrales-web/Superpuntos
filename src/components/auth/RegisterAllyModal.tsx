...
export const RegisterAllyModal: React.FC<RegisterAllyModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const [createdSystemEmail, setCreatedSystemEmail] = useState('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      triggerConfetti();
      const generatedEmail = res.user?.systemEmail || generateAcronymicEmail(name.trim(), documentId.trim(), businessName.trim());
      setCreatedSystemEmail(generatedEmail);
      setIsSuccess(true);
    } catch (err: any) {
      setError(err?.message || 'Error al procesar la solicitud del usuario.');
    } finally {
      setIsSubmitting(false);
    }
  };
      ...
      >
        ...
        {isSuccess ? (
          <div className="p-6 sm:p-8 text-center space-y-4">
            ...
            <div>
              <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider bg-blue-50 text-blue-900 border border-blue-200">
                <Clock className="w-3.5 h-3.5 text-blue-900" /> Solicitud Enviada a Red de Aliados
              </span>
              ...
              <p className="text-xs sm:text-sm text-slate-600 mt-1 max-w-sm mx-auto leading-relaxed">
                Tu solicitud ha sido enviada al administrador en la <strong>Red de Aliados</strong> para habilitar tu acceso en la base de datos.
              </p>
            </div>
            ...
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200 text-left space-y-2.5 text-xs">
              <div className="p-2.5 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 flex items-start gap-2">
                ...
                <span>
                  El administrador agregará tus credenciales a la <strong>base de datos</strong>. Muy pronto te enviaremos la información de acceso.
                </span>
              </div>

              {createdSystemEmail && (
                <div className="mt-2 p-3 rounded-xl bg-blue-50/80 border border-blue-200">
                  <span className="text-[10px] uppercase font-bold text-blue-900 tracking-wider block">
                    Correo Institucional Asignado Acrónicamente
                  </span>
                  <div className="flex items-center justify-between gap-2 mt-1">
                    <span className="font-mono font-bold text-xs sm:text-sm text-blue-950 break-all">
                      {createdSystemEmail}
                    </span>
                    <button
                      type="button"
                      onClick={() => {
                        navigator.clipboard.writeText(createdSystemEmail);
                        setCopiedEmail(true);
                        setTimeout(() => setCopiedEmail(false), 2000);
                      }}
                      className="px-2 py-1 rounded-lg bg-white border border-blue-300 text-blue-900 text-xs font-bold flex items-center gap-1 shrink-0 shadow-2xs hover:bg-blue-100 cursor-pointer"
                    >
              ...
              <p className="text-[11px] text-slate-500 pt-1">
                🔐 Todos los usuarios registrados en la base de datos tienen acceso completo al sistema mediante su correo y contraseña.
              </p>
            </div>
            ...
            <button
              ...
              className="w-full py-3 px-6 rounded-xl font-bold text-xs sm:text-sm bg-slate-900 hover:bg-slate-800 text-white shadow-md cursor-pointer transition-all"
            >
              Entendido — Cerrar
            </button>
          </div>
